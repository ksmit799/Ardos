...
void DistributedObject::HandleDatagram(const std::shared_ptr<Datagram>& dgIn) {
  ...
  switch (msgType) {
    ...
    case STATESERVER_OBJECT_GET_AI_RESP: {
      ...
      if (parentId != _parentId) {
        spdlog::get("ss")->warn(
            "Distributed Object: '{}' received AI "
    ...
    case STATESERVER_OBJECT_CHANGING_LOCATION: {
      ...
      if (newParent == _doId) {
        ...
        spdlog::get("ss")->warn(
            "Distributed Object: '{}' received changing "
    ...
    case STATESERVER_OBJECT_GET_LOCATION: {
      ...
      auto dg = std::make_shared<Datagram>(
          sender, _doId, STATESERVER_OBJECT_GET_LOCATION_RESP);
    ...
    case STATESERVER_OBJECT_GET_ZONES_OBJECTS: {
      ...
      spdlog::get("ss")->debug(
          "Distributed Object: '{}' handling get "
      ...
      if (queriedParent == _parentId) {
        // Query was relayed from parent! See if we match any of the zones and
        // if so, reply.
...
bool DistributedObject::HandleOneUpdate(DatagramIterator& dgi,
  ...
  if (!field) {
    spdlog::get("ss")->error(
        "Distributed Object: '{}' received field "
